...
type VLESSConfig struct {
	...
	// Reality specific
	privateKey string // Server-side private key (for inbound configuration)
	publicKey  string // Client-side public key (for outbound/subscription)
	shortID    string
	spiderX    string
}
...
func NewVLESSConfig(
	...
	// Note: privateKey, publicKey, and shortID can be auto-generated by use case if not provided
	// But if any of them is provided, all must be provided for consistency
	if security == VLESSSecurityReality {
		hasPrivateKey := privateKey != ""
		hasPublicKey := publicKey != ""
		hasShortID := shortID != ""

		// If any Reality field is provided, all required fields must be provided
		if hasPrivateKey || hasPublicKey || hasShortID {
			if !hasPrivateKey {
				return VLESSConfig{}, fmt.Errorf("private_key is required when public_key or short_id is provided for Reality security")
			}
			if !hasPublicKey {
				return VLESSConfig{}, fmt.Errorf("public_key is required when private_key or short_id is provided for Reality security")
			}
			if !hasShortID {
				return VLESSConfig{}, fmt.Errorf("short_id is required when private_key or public_key is provided for Reality security")
			}
		}
		// If none provided, use case will auto-generate them
	}
...
// PrivateKey returns the Reality private key (for server-side inbound configuration)
func (vc VLESSConfig) PrivateKey() string {
	return vc.privateKey
}

// PublicKey returns the Reality public key (for client-side outbound/subscription)
...
func (vc VLESSConfig) Equals(other VLESSConfig) bool {
	return vc.transportType == other.transportType &&
		...
		vc.privateKey == other.privateKey &&
